...
unsigned int pna_net_lookup(unsigned int ip)
{
  ...
  unsigned int curnet = 0xFFFFFFFF;
...
int pna_net_init(void)
{
  ...
  if(!pna_net_head){
    printk("failed to init net head\n");
    return -1;
  } 

  net_proc_node = create_proc_entry(DTRIE_PROC_STR, 0644, proc_parent);
  if(!net_proc_node){
  ...
  net_proc_node->uid = 0;
  net_proc_node->gid = 0;
  net_proc_node->gid = sizeof(unsigned int)*3;
